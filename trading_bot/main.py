...
from trading_bot import signal_logger
...
def run_single_analysis(
    ...
    try:
        ...
        if signals:
            ...
            signal_logger.log_signals_to_db(signals, symbol, db_path=db_path)
            if alert_mode:
                for s in signals:
                    send_alert(s)
        return signals
        ...
        def _iteration_body():
            for sym in symbols:
                ...
                for signal in signals[-3:]:  # show last few
                    ...
                    if signal_logger.mark_signal_handled(
                        sym,
                        strategy,
                        timeframe,
                        ts,
                    ...
                    if live_trade and exchange:
                        ...
                        try:
                            if broker:
                                ...
                                signal_logger.log_trade_to_db(trade, db_path=db_path)
                            elif portfolio:
                                if action == "buy":
