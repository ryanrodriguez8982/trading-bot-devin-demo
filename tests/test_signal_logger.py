...
def test_log_signals_to_db(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM signals WHERE symbol = ? AND strategy_id = ?", (test_symbol, test_strategy)
        )
...
def test_get_signals_from_db(tmp_path):
    ...
    retrieved_signals = get_signals_from_db(
        symbol="ETH/USDT", strategy_id="test_strategy", limit=2, db_path=str(db_path)
    )
...
def test_log_signals_default_path(monkeypatch, tmp_path):
    monkeypatch.setattr(signal_logger, "_default_db_path", lambda: str(tmp_path / "signals.db"))
...
def test_database_schema(tmp_path):
    ...
    signals = [{"timestamp": pd.Timestamp("2024-01-01 14:00:00"), "action": "buy", "price": 54000.0}]
...
def test_malformed_signal_entry(tmp_path):
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS signals (timestamp TEXT, action TEXT, price REAL, symbol TEXT, strategy_id TEXT)"
    )
    conn.close()
...
def test_mark_signal_handled(tmp_path):
    ...
    first = mark_signal_handled("BTC/USDT", "sma", "1m", "123", "buy", db_path=str(db_path))
    second = mark_signal_handled("BTC/USDT", "sma", "1m", "123", "buy", db_path=str(db_path))
    assert first is False
    assert second is True


def test_mark_signal_handled_default_path(monkeypatch, tmp_path):
    monkeypatch.setattr(signal_logger, "_default_db_path", lambda: str(tmp_path / "proc.db"))
...
def test_get_signals_default_path(monkeypatch, tmp_path):
    monkeypatch.setattr(signal_logger, "_default_db_path", lambda: str(tmp_path / "sigs.db"))
