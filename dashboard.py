...
st.markdown("Visualize trading signals and price data with SMA crossover analysis")
...
strategy_options = ["All", "sma"]
selected_strategy = st.sidebar.selectbox("Strategy", strategy_options, index=0)

limit = st.sidebar.slider("Number of signals to display", min_value=10, max_value=500, value=50, step=10)

sma_short = st.sidebar.number_input("Short SMA Period", min_value=1, max_value=50, value=5)
sma_long = st.sidebar.number_input("Long SMA Period", min_value=1, max_value=200, value=20)
...
with col1:
    ...
    def create_mock_data(symbol, limit=500):
        ...
        timestamps = [datetime.now() - timedelta(minutes=i) for i in range(limit, 0, -1)]
    ...
    try:
        with st.spinner("Fetching price data..."):
            try:
                df = fetch_btc_usdt_data(symbol=selected_symbol, timeframe="1m", limit=500)
        ...
        if not df.empty:
            df_copy = df.copy()
            df_copy[f'sma_{sma_short}'] = df_copy['close'].rolling(window=sma_short).mean()
            df_copy[f'sma_{sma_long}'] = df_copy['close'].rolling(window=sma_long).mean()
            
            signals = sma_crossover_strategy(df_copy, sma_short=sma_short, sma_long=sma_long)
            
            fig, ax = plt.subplots(figsize=(12, 6))
            
            ax.plot(df_copy['timestamp'], df_copy['close'], label='Price', color='black', linewidth=1)
            ax.plot(df_copy['timestamp'], df_copy[f'sma_{sma_short}'], label=f'SMA {sma_short}', color='blue', alpha=0.7)
            ax.plot(df_copy['timestamp'], df_copy[f'sma_{sma_long}'], label=f'SMA {sma_long}', color='red', alpha=0.7)
            ...
            ax.set_title(f'{selected_symbol} Price with SMA Crossover Signals')
            ax.legend()
            ...
            if signals:
                ...
                if st.button("Save Signals to Database"):
                    try:
                        log_signals_to_db(signals, selected_symbol, 'sma')
...
with col2:
    ...
    try:
        ...
        if signals_data:
            signals_df = pd.DataFrame(signals_data, columns=['Timestamp', 'Action', 'Price', 'Symbol', 'Strategy'])
            ...
            styled_df = signals_df_display.style.apply(highlight_action, axis=1)
